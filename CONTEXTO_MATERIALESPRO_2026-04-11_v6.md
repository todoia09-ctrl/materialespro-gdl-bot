...
## Fecha: 2026-04-11 v6 (Import Stock fix + Stock verificado 804 productos)
...
36. **REGLA #36:** Al insertar nuevos endpoints en api.js via script — NUNCA usar contador de `});` para encontrar cierre de bloque. Insertar ANTES de `module.exports = router` como posición segura garantizada.
...
38. **REGLA #38:** Scripts de fix deben incluir DRY-RUN: imprimir resultado en consola ANTES de escribir al disco. Verificar que el patrón OLD aparece exactamente 1 vez antes de reemplazar.
...
- **Stock = 50 en TODOS los productos** (cargado 2026-04-11 — test inicial)
- stock_minimo = 10 en todos
- Columnas: id, producto_id, nombre, stock, stock_minimo, unidad, actualizado_en, actualizado_por, categoria, marca, presentacion, precio_venta
- ⚠️ NO tiene columna `codigo` — join via producto_id → catalogo_productos.id
...
| `e5641d1` | fix: importar-stock 404 — extraer endpoint del interior del PATCH callback |

---

## 🐛 BUGS RESUELTOS SESIÓN v6

| Bug | Causa raíz | Fix |
|---|---|---|
| importar-stock 404 | router.post quedó dentro del callback del PATCH /:id (contador `});` falló) | Scripts fix_importar_stock_v2 + fix_patch_catch_v2 + fix_orphan_catch_v2 |
| Scripts fix con múltiples iteraciones | No se hacía dry-run ni verificación de patrón único antes de escribir | Nueva REGLA #38: dry-run obligatorio |
...
| # | Prioridad | Tarea |
|---|---|---|
| 1 | 🔴 | **Fix bot marca:** cliente dice "Perdura" → bot responde "Pegaduro" (matching de marca en system prompt) |
| 2 | 🔴 | **Fix horario en todos los mensajes:** debe ser `🕐 Lun–Vie 8am–6pm · Sáb 8am–2pm` (actualmente dice Lun-Sab 8am-6pm) |
| 3 | 🔴 | **Fix bug pedido** — `132x 132 × Perdura...` (qty duplicada en items_json) |
| 4 | 🔴 | **Cargar stock real** — reemplazar stock=50 de prueba con cantidades reales por producto |
| 5 | 🟡 | **Prueba pedido real WhatsApp** con SIKA — verificar cálculo m², filtro marca, precio por nivel |
| 6 | 🟡 | **Verificar flujo precio nivel** 1/2/3/4 con datos reales |
...
| 9 | 🟢 | **Migración Railway o Render Starter** — post-testing |

---

## ⚠️ LECCIONES CRÍTICAS SESIÓN v6

### REGLA #38 — Dry-run obligatorio en scripts de fix
- Antes de `fs.writeFileSync`: imprimir OLD encontrado, NEW resultado, y contar ocurrencias
- Si `api.split(OLD).length - 1 !== 1` → ABORT, no escribir
- Proceso correcto: Leer → Verificar patrón único → Dry-run en consola → Escribir → node --check

### Schema tabla inventario — columnas reales
```
id | producto_id | nombre | stock | stock_minimo | unidad |
actualizado_en | actualizado_por | categoria | marca | presentacion | precio_venta
```
⚠️ NO existe columna `codigo` — join siempre via producto_id

### Bot — comportamiento stock (observado en prod)
- Bot dice "no puedo confirmar existencias en tiempo real" aunque stock está en DB
- Causa probable: `buildCatalogText()` o `buildSystemPrompt()` no pasa campo `stock` al contexto IA
- Pendiente revisar qué campos del inventario se incluyen en el prompt

---

## 📋 TABLA PROPAGACIÓN — Estado post-sesión v6
...
| `verify_stock_import.js` | Verifica stock en inventario DB por marca (usa producto_id, no codigo) |
...
| POST | `/api/inventario/importar-stock` | Import masivo Excel (base64) → codigo+stock+stock_minimo ✅ FIXED |
...
> "Eres mi Senior CTO y principal engineer para MaterialesPro GDL. Adjunto CONTEXTO_MATERIALESPRO_2026-04-11_v6.md con estado completo. Lee y confirma antes de continuar."
