/**
 * fix_stock_bulk_v3.js
 * MaterialesPro GDL — Carga stock_physical 818 productos
 * Fix: columna es "codigo" (no sku) en catalogo_productos
 *
 * EJECUTAR:
 *   node C:\Projects\materialespro-enterprise-v10\materialespro-enterprise-v10\whatsapp-bot-gdl\fix_stock_bulk_v3.js
 */
...
async function run() {
  ...
  console.log("\n[FIX STOCK v3] Conectado. " + STOCK_DATA.length + " productos.\n");
    ...
    for (const item of STOCK_DATA) {
      try {
        const r = await client.query(
          "UPDATE inventario " +
          "SET stock_physical = $1, stock_minimo = $2, actualizado_en = NOW() " +
          "WHERE catalogo_id = (SELECT id FROM catalogo_productos WHERE codigo = $3 LIMIT 1)",
          [item.stock, item.stock_minimo, item.codigo]
        );
        if (r.rowCount === 0) {
          notFound++;
          if (notFound <= 10) console.warn("  [NOT FOUND] " + item.codigo);
        } else {
          ok++;
        }
        if (ok % 100 === 0 && ok > 0) console.log("  -> " + ok + "/" + STOCK_DATA.length);
      } catch (e) {
        fail++;
        if (fail <= 5) console.error("  [ERR] " + item.codigo + ": " + e.message);
      }
    }
  } finally {
    client.release();
  ...
  console.log("TOTAL:     " + STOCK_DATA.length + "\n");
}
