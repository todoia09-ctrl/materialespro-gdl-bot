/**
 * fix_stock_bulk_v2.js
 * MaterialesPro GDL — Carga stock_physical 818 productos via pg + DATABASE_URL
 *
 * EJECUTAR:
 *   node C:\Projects\materialespro-enterprise-v10\materialespro-enterprise-v10\whatsapp-bot-gdl\fix_stock_bulk_v2.js
 */
...
async function run() {
  ...
  console.log("\n[FIX STOCK v2] Conectado. " + STOCK_DATA.length + " productos.\n");
    ...
    for (const item of STOCK_DATA) {
      try {
        const r = await client.query(
          `UPDATE inventario
             SET stock_physical = $1,
                 stock_minimo   = $2,
                 actualizado_en = NOW()
           WHERE catalogo_id = (
             SELECT id FROM catalogo_productos WHERE sku = $3 LIMIT 1
           )`,
          [item.stock, item.stock_minimo, item.codigo]
        );
        if (r.rowCount === 0) {
          notFound++;
          if (notFound <= 5) console.warn("  [NOT FOUND] " + item.codigo);
        } else {
          ok++;
        }
